...
//  http://localhost:9090/BugApp/employee
	...
	/*
	 * @GetMapping(path = "byName/{empName}", produces =
	 * MediaType.APPLICATION_JSON_VALUE) public List<Employee>
	 * getAllEmployeeByName(@PathVariable("empName") String empName) { return
	 * employeeService.getAllEmployeeByName(empName); }
	 */
	
	@GetMapping(path = "bynameOrId/{empName}",produces = "application/json")
	public List<Employee> getAllEmployeesByNameOrId(@Valid @PathVariable("empName") String empName) {
		List<Employee> result = employeeService.getAllEmployeesByNameOrId(empName);
		return result;
	}
